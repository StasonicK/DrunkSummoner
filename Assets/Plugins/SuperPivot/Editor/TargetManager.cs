...
namespace SuperPivot
{
    internal class TargetManager
    {
        ...
        {
            if (FoldableHeader.Begin("Bounds"))
            {
        ...
        {
            foreach (var t in m_Targets)
                t.SetPivotPosition(t.LerpBoundsPosition(sliderClampedValue), API.Space.Global);
        }

        void SetPivotLerpBoundsPosition(TargetWrapper.Component comp, float sliderClampedValue)
        {
            foreach (var t in m_Targets)
                t.SetPivotPosition(comp, t.LerpBoundsPosition(comp, sliderClampedValue), API.Space.Global);
        }
    }
}